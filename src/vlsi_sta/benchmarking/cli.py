...
from .plotting import BenchmarkPlotError, plot_evaluation


def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="vlsi-sta benchmark",
    ...
    plot = commands.add_parser("plot", help="plot a benchmark evaluation run")
    plot.add_argument("evaluation_directory", type=Path)
    plot.add_argument(
        "--output-dir",
        type=Path,
    ...
    if arguments.log_level == "INFO":
        logging.getLogger("vlsi_sta").setLevel(logging.WARNING)
        logging.getLogger("vlsi_sta.benchmarking").setLevel(logging.INFO)
    try:
        if arguments.command == "generate":
            ...
            output = arguments.output_dir or arguments.evaluation_directory / "plots"
